 ...
 * <p>
 * These come in two flavors: {@code Addition}s and {@code Removal}. {@code Addition}s cover a
 * single index and represents the fact that the containing PatchWorkArray has a sublist at that
 * index in its backingList.  {@code Removal}s cover a continuous block of indices in the {@code
 * PatchWorkArray}'s backingList and represent that those indices do not contain valid elements.
 */
abstract class Alteration implements Comparable<Alteration> {
  ...
  /**
   * Returns the impact on performance which the existence of this Alteration will have on a
   * containing {@code PatchWorkArray}.
   * <p>
   * Let <i>t</i> be the sum of the values returned by calling this method on all {@code
   * Alteration}s in a {@code PatchWorkArray}, the time it takes to find an element in that list by
   * index should be <i>O(log(t))</i>.
   *
   * @return an indication of the performance hit a {@code PatchWorkArray} would have if it included
   * this {@code Alteration}.
   */
  abstract int getPerformanceHit();

  /**
   * Returns whether or not this {@code Alteration} covers the given index in the {@code
   * PatchWorkArray}'s backingList.
   *
  ...
  /**
   * Returns the number of elements that this alteration affects, which is calculated as being the
   * absolute size of the {@code #indexDiff}.
   *
...
/**
 * Represents an alteration to a patch work array, where elements have been added to the array at a
 * specific index.
 */
...
class Removal extends Alteration {
  ...
  /**
   * This should only be called if the element to in position index-1 is a normal element that is
   * due to be removed.
   */
