...
/**
 * SortedList is an implementation of a {@link List}, backed by an AVL tree. Does not support any
 * optional operations, with the exception of: {@code remove(int)}, {@code remove(Object)}, {@code
 * clear} and {@code add()}.
 * <p>
...
public class SortedList<T> extends AbstractList<T> implements Serializable {
  ...
  /**
   * Constructs a new, empty SortedList which sorts the elements according to the given {@code
   * Comparator}.
   *
  ...
  /**
   * Inserts the given object into this {code SortedList} at the appropriate location, so as to
   * ensure that the elements in the list are kept in the order specified by the given {@code
   * Comparator}.
   * <p>
   ...
   * <p>
   * This method can be overridden by a subclass in order to change the definition of the {@code
   * Node}s that this List will store.
   * <p>
  ...
  /**
   * Tests if this tree has exactly the same structure and values as the given one, should only be
   * used for testing.
   *
   * @param other the {@code SortedList} to compare this to.
   * @return true if and only if all this {@code SortedList} are structurally the same with each
   * node containing the same values.
   */
   ...
   *
   * @return an {@code Iterator} which provides the elements of this {@code SortedList} in the order
   * determined by the given {@code Comparator}.
   */
  @Override
  ...
  /**
   * Returns whether or not the given object is present in this {@code SortedList}. The comparison
   * check uses the {@code Object#equals(Object)} method and work under the assumption that the
   * given <code>obj</code> must have type <code>T</code> to be equal to the elements in this {@code
   * SortedList}.  Works in time
  ...
  /**
   * Returns the node representing the given value in the tree, which can be null if no such node
   * exists.
   * <p>
  ...
  /**
   * Removes the first element in the list with the given value, if such a node exists, otherwise
   * does nothing.  Comparisons on elements are done using the given comparator.
   * <p>
  ...
  /**
   * Removes the given {@code Node} from this {@code SortedList}, re-balancing if required, adds to
   * <code>modCount</code> too.
   * <p>
  ...
  protected Node findNodeAtIndex(int index) {
    ...
    while (current != null) {  //should always break, due to constraint above..
      ...
      if (totalSmallerElements > index) { //go left..
        ...
        totalSmallerElements -=
            (current.rightChild == null) ? 0 : current.rightChild.sizeOfSubTree();
      } else { //go right..
        totalSmallerElements++;
        current = current.rightChild;
  ...
  /**
   * Copies the elements in the {@code SortedList} to the given array if it is large enough to store
   * them, otherwise it returns a new array of the same type with the elements in it.
   * <p>
   ...
   *
   * @return an array representation of this list.
   * @throws ClassCastException in the case that the provided array can not hold objects of this
   * type stored in this {@code SortedList}.
   */
  @SuppressWarnings("unchecked")
  @Override
  ...
  /**
   * Returns the smallest balance factor across the entire list, this serves not other purpose other
   * than for testing.
   *
  ...
  /**
   * Returns the largest balance factor across the entire list, this serves not other purpose other
   * than for testing.
   *
  ...
  protected class Node implements Comparable<Node> {
    ...
    /**
     * The unique id of this node: auto generated from the containing tree, the newer the node the
     * higher this value.
     */
    ...
    /**
     * Returns whether or not this not is the left child of its parent node; if this is the root
     * node, then <code>false</code> is returned.
     *
    ...
    /**
     * Returns whether or not this not is the right child of its parent node; if this is the root
     * node, then <code>false</code> is returned.
     *
    ...
    /**
     * Compares the value stored at this node with the value at the given node using the comparator,
     * if these values are equal it compares the nodes on their IDs; older nodes considered to be
     * smaller.
     *
     * @return if the comparator returns a non-zero number when comparing the values stored at this
     * node and the given node, this number is returned, otherwise this node's id minus the given
    ...
    /**
     * Gets the next biggest node in the tree, which is <code>null</code> if this is the largest
     * valued node.
     *
     * @return the next biggest node in the tree, which is <code>null</code> if this is the largest
     * valued node.
     */
    ...
    /**
     * Gets the node that is the next smallest in the tree, which is <code>null</code> if this is
     * the smallest valued node.
     *
     * @return the node that is the next smallest in the tree, which is <code>null</code> if this is
     * the smallest valued node.
     */
    ...
    /**
     * Returns whether or not this {@code Node} is a leaf; this is true in the case that both its
     * left and right children are set to <code>null</code>.
     *
    ...
    /**
     * Returns, the number of children of this {@code Node} plus one.  This method uses a cached
     * variable ensuring it runs in constant time.
     *
    ...
    /**
     * Updates the height and the number of children for nodes on the path to this. Also calls
     * {@code #updateAdditionalCachedValues()}, for every node on the path to this, including this
     * one.
     */
    ...
    /**
     * Called when a node is inserted or removed from the tree and provides a hook for sub-classes
     * to get their cached values updated.
     * <p>
     * This method is called every time the list is altered.  It is first called on the deepest node
     * which is affected by a given change and is then subsequently called on ancestors of this node
     * until it is called on the root node.
     * <p>
