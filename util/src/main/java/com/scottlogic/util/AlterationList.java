...
/**
 * List to store {@code Alteration}s to the backingList of a {@code PatchWorkArray}, so that
 * elements can be accessed quickly.
 */
class AlterationList extends NaturalSortedList<Alteration> {
  ...
  /**
   * Deletes the alteration covering the given index in the {@code PatchWorkArray}'s backingList.
   *
   * @param index the index of the backingList for which the Alteration needs to be removed. Note
   * that in the case of a {@code Removal}, the index only needs to be covered
  ...
  void deleteAlterationAtIndex(final int index) {
    ...
    if (alt == null || !alt.coversBackingListIndex(index)) {
      throw new IllegalStateException(
          "Can not delete alteration covering index: " + index + " as no such " +
              "alteration, exists!");
    }
    ...
    if (alt.getClass() == Addition.class) {
      ...
      if (alt.size() == 1) {
        ...
        if (alt.index + alt.size() - 1
            == index) { //case that we're removing "from the end of the Removal"
          alt.indexDiff--;
  ...
  /**
   * Obtains the {@code AlterationNode} that contains the {@code Alteration} covering the given
   * index or <code>null</code> in the case that no such node exists.
   * <p>
   * Takes time <i>O(log(size())</i>.
   *
   * @param index the covered index to locate the {@code AlterationNode} for.
   * @return the {@code AlterationNode} covering the given index, or <code>null</code> if no such
   * node exists.
   */
  ...
  /**
   * Moves the {@code Removal} at the given index one position to the left and increments its size
   * by one, this should be used when removing an element to the left of the this {@code Removal}.
   *
   * @param index the index of the {@code Removal}.
   * @throws ClassCastException in the case that the Alteration at the given index is not a {@code
   * Removal}.
  ...
  /**
   * Gets the Removal which covers the given index, decreases its size by one and shifts its index
   * one place to the right.
   * <p>
   * This should be used in the case that you wish to add an element at the position held by the
   * start of the {@code Removal} covering the given index and the {@code Removal} has size at least
   * two.
   *
   * @param index the index of the {@code Removal}.
   */
   ...
   * <p>
   * This should be used in the case that you wish to remove the element to the right of this {@code
   * Removal}.
   *
   * @param index the index of the {@code Removal}.
   */
   ...
   * <p>
   * This should be used in the case that you wish to add an element at the position held by the end
   * of the {@code Removal} covering this index and the {@code Removal} has size at least two.
   *
   * @param index the index of the {@code Removal}.
   */
  ...
  /**
   * Returns an Iterator which begins at the given index in this {@code AlterationList}.
   *
   * @param startIndex the index in this list to start at, for example giving 0 is equivalent to
   * calling {@code #iterator()} in a non-empty list.
  ...
  /**
   * Gets the Alteration at the given index and adds the  <code>indexDiffChange</code> to its
   * <code>indexDiff</code>.
   * <p>
   * Note that the <code>indexDiffChange</code> should be negative if you are increasing the size of
   * an {@code Addition} and positive when increasing the size of a {@code Removal}.
   *
  ...
  /**
   * Gets the location of the element at the given index in the backingList that this {@code
   * AlterationList} is storing the changes to.
   * <p>
  ...
  /**
   * Returns the total "performance hit" of the {@code Alteration}s stored in this {@code
   * AlterationList}.
   * <p>
   * This method make use of a cached value and hence takes constant time.
   *
   * @return the total "performance hit" of the {@code Alteration}s stored in this {@code
   * AlterationList}.
   */
  ...
  protected void remove(Node node) {
    totalPerformanceHit -= node.getValue().getPerformanceHit();
    super.remove(node);
  }
