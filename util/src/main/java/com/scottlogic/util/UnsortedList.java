 ...
 * <p>
 * Performs the operations: {@code add(int, Object)}, {@code get} and {@code remove(int)} operations
 * in in time <i>O(log(n))</i> and the {@code contains(Object)} and {@code remove(Object)}
 * operations in time <i>O(n)</i>, where <i>n</i> is the number of elements in the list.
 * <p>
...
public class UnsortedList<T> extends SortedList<T> {
  ...
  /**
   * Returns whether or not the given object whether or not the given object is present in this
   * {@code UnsortedList}.
   * <p>
   * This implementation takes <i>O(n)</i> time, where <i>n</i> is the number of element in this
   * list.
   *
   ...
   * does nothing.  Works in time <i>O(n)</i>, where
  ...
  public boolean remove(Object value) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (nodeAtIndex.getValue()
          .equals(value)) { //nulls not allowed in the list, so don't need to deal with that case.
        remove(nodeAtIndex);
        treeAltered = true;
        modCount++;
        break;
  ...
  /**
   * Adds the given element to the head of this list. Shifts the elements currently in the list (if
   * any) to the right (adds one to their indices).
   * <p>
   * Works in time <i>O(log(n))</i>, where <i>n</i> is the number of elements in the list.
   *
   * @param element the object to by inserted.
  ...
  /**
   * Class representing the individual nodes of an unsorted list extends the regular SortedList.Node
   * class by storing the position of the node in the tree.
   */
  protected class UnsortedNode extends Node {
    ...
    /**
     * Constructs a new {@code UnsortedNode}  which should be positioned at the given index when
     * inserted into the tree.
     *
     * @param obj the object to store at the node {@code UnsortedNode}.
     * @param initialIndex the index which this {@code UnsortedNode} should be inserted into the
     * list in.
     */
    ...
    /**
     * Gets the index of this node in the list, using the cached value if it is not stale and
     * otherwise calculating it from the ancestor nodes and setting the cached value.
     */
    ...
    protected int getIndexInList() {
      ...
      while (!path.isEmpty()) {
        current = path.pop();
        UnsortedNode parent = (UnsortedNode) current
            .getParent(); //this parent can only be fresh or null..
        if (parent == null) { //root case..
          ...
          if (current.isLeftChildOfParent()) {
            ...
            current.indexInList =
                parent.indexInList - (rightChild == null ? 1 : 1 + rightChild.sizeOfSubTree());
          } else { //current is right child of parent case..
            Node leftChild = current.getLeftChild();
            current.indexInList =
                parent.indexInList + (leftChild == null ? 1 : 1 + leftChild.sizeOfSubTree());
          }
        }
    ...
    /**
     * Compares this node with the given node - which must be an {@code UnsortedNode} instance in
     * the same {@code UnsortedList}.  The comparison is based on the current position of the nodes
     * in the list.
     * <p>
     * Nodes with equal indices are compared on whether they are in the tree yet or not; those in
     * the tree are considered to be larger.
     *
