...
public class PatchWorkArray<T> extends AbstractList<T> implements Serializable {
  ...
  /**
   * Creates a new {@code PatchWorkArray} backed by an ArrayList with an initial capacity of ten.
   */
  ...
  /**
   * Creates a new {@code PatchWorkArray} backed by an ArrayList with the given initial capacity.
   */
  ...
  /**
   * Obtains the element at the given index in this {@code PatchWorkArray}.  Note that indices are
   * numbered from zero through to {@code #size()-1}.
   * <p>
   * This implementation works in time <i>O(log(d))</i>, where <i>d</i> is the number of alterations
   * in the list.
   *
  ...
  /**
   * Adds the given object to the end of this {@code PatchWorkArray}. This implementation forbids
   * <code>null</code> values from being added, an {@code IllegalArgumentException} is thrown in the
   * case that this is attempted.
   *
  ...
  /**
   * The performance hit caused by the {@code Alteration}s to the backingList of this {@code
   * PatchWorkArray}.  The time it takes to perform the operations: get, remove, add is the log of
   * this value.
   *
  ...
  /**
   * Testing method only - verifies that the alterations List and the backing list are in sync.
   *
   ...
   * indices).
   * <p>
   * If the given index is the current size of the list, the {@code #add(Object)} method is invoked,
   * otherwise the element is lazily added and the number of alterations is incremented.
   *
  ...
  public void add(int index, T obj) {
    ...
    if (index < 0 || index > size()) {
      throw new IndexOutOfBoundsException(index
          + " is not a valid index for inserting an element, must be between 0 and size() inclusive.");
    }
    ...
    if (index == size()) {
      ...
      if (loc.hasSubListIndex()) {
        ...
        if (blIndex > 0 && slIndex == 0
            && getTypeAtBackingListIndex(blIndex - 1) == ElementType.NO_VALUE) {
        ...
        } else if (blIndex < backingList.size() - 1
            && getTypeAtBackingListIndex(blIndex + 1) == ElementType.NO_VALUE) {
  ...
  /**
   * Pushes all the alterations in this {@code PatchWorkArray} to the backing list, leaving the
   * number of alterations as zero.
   * <p>
   * This method performs structural changes to the list and hence increments the {@code modCount}
   * of the list.
   */
  public void fix() {
  ...
  /**
   * Returns the element type of the given object, which should be something from the backing list.
   *
   ...
   * <p>
   * This method can increase or decrease the number of alterations, depending on the element on
   * which it is called.
   *
      ...
      if (blIndex == backingListLastIndex) {
        ...
        if (blIndex > 0 && getTypeAtBackingListIndex(blIndex - 1) == ElementType.NO_VALUE) {
          ...
          if (removalToRight) { //case that there is a removal to both sides (merge Removals)..
            ...
            int rightRemovalSize = rightRemoval.getValue()
                .size(); //need to do this first so it doesn't change..
            ...
            alterations
                .changeIndexDiffForAlterationAtIndex(leftHandRemovalIndex, rightRemovalSize + 1);
  ...
  private class Itr implements Iterator<T> {
          ...
          case NO_VALUE: //its a removal - figure out how far to skip forward..
            ...
            switch (getTypeAtBackingListIndex(nextBLIndex)) {
              ...
              case NORMAL:
                ...
                break;
              case SUB_LIST:
                lastSubListNode = ((UnsortedList<T>) backingList.get(nextBLIndex))
                    .findNodeAtIndex(0);
                next = lastSubListNode.getValue();
            }
            break;
    ...
    public void remove() {
      ...
      if (nextIndex == size) {
        ...
        lastSubListNode = ((UnsortedList<T>) backingList.get(loc.backingListIndex))
            .findNodeAtIndex(loc.subListIndex);
      }
